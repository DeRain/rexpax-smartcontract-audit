...
contract AdvancedToken is ERC20, owned {
    ...
    function withdrawEther(uint256 _value) public onlyOwner {
        require(this.balance > 0 && this.balance >= _value);
...
contract ICO is AdvancedToken {
    ...
    enum State { Disabled, Presale, waitingForICO, ICO, Active }
    ...
    function ICO() internal payable owned {
        require(contract_state == State.Disabled);
        totalSupply = 0;
        // For the tests replace 1511877600 to "now"
        startTime = 1512482400; // 5 dec 2017 9:00AM EST (Tue, 05 Dec 2017 14:00:00 GMT)
        presaleMaxSupply = 190000000;
        marketMaxSupply = 1260000000;
        contract_state = State.Presale;
    ...
    function () private payable crowdsaleState {
        ...
        uint256 restTokens = currentMaxSupply * 1 ether - totalSupply;
